...
## What is a RUM Server
The RUM Server is a backend part of the Canarytrace RUM (Real User Monitoring). It is responsible for receiving and processing data from the RUM Clients and storing that data in [Elasticsearch](./elasticsearch) for later analysis.

Once the RUM Server is up, you can configure the [RUM Client](./rumClient) to start sending data to the RUM Server. This typically involves adding a small piece of code to your website or application that initializes the [RUM client](./rumClient) and configures it to send data to the server.


import LicenseInfo from '../../src/components/LicenseInfo';

<LicenseInfo app="Canarytrace RUM" />
...
The RUM Server is distributed via a Docker image and exposes several API endpoints, including one for getting the [RUM Client](./rumClient) JavaScript, another for receiving data from the [RUM Client](./rumClient), and additional endpoints for the browser CSP reporter. The RUM Server stores incoming data to an [Elasticsearch](./elasticsearch) database.
...
### Services
Services provide a load balancer between PODs running the RUM Server and expose an external IP for public use. You can run one or more PODs with the RUM Server depending on the traffic your pages receive. Running more PODs with the RUM Server can collect more data from the [RUM Client](./rumClient).

### Elasticsearch and Kibana

[Elasticsearch](https://www.elastic.co/elasticsearch/) is a database for the data collected by Canarytrace RUM. [Kibana](https://www.elastic.co/kibana/) is a web interface for viewing and searching this data in [Elasticsearch](./elasticsearch), as well as for creating graphs, visualizations, and dashboards.
...
|-|-|-|
|`GET`|`/rum`|The init script calls this endpoint to obtain the [RUM Client](./rumClient)|
|`POST`|`/rum`|The [RUM Client](./rumClient) sends payloads to this endpoint.|
...
|`POST`|`/rum/webrtc`|Endpoint for sending the WebRTC metrics. More information in the [WebRTC](./rumClient#webrtc) section.|
...
The [RUM Client](./rumClient) does not set up or manage CSP / that is entirely up to the web browser.
...
The resource requirements depend on the amount of data that the [RUM Client](./rumClient) sends to the RUM Server, including the number of captured requests, user actions, events, and so on.
...
- Measure used resources by the RUM Server on your Kubernetes cluster. If the RUM Server use 85% resources, please run the second instance ensure optimal performance. Alternatively, you could optimize the amount of data arriving from the [RUM Client](./rumClient).
...
3. `secret.yaml` [Secret](https://kubernetes.io/docs/concepts/configuration/secret/) contains auth to the [Elasticsearch](./rumClient), as well as a license for using the RUM Server.
...
### Log
The RUM Server logs all interesting information. This includes access to your pages, endpoints, and the size of the payload received from the [RUM Client](./rumClient).

#### Syntax
`dateTime | method | endpoint | href | size` and information about the queue with payloads received from the [RUM Client](./rumClient) and how they are sent to [Elasticsearch](./elasticsearch).
...
|`ENABLE_WEBRTC_METRICS_COLLECTION`|`false`|This environment variable enable the function for collecting WebRTC metrics in the [RUM Client](./rumClient)|

## Restrict traffic for specific URLs

If you want to block traffic from a specific domain (e.g., test.company.com) and allow traffic from another (e.g., company.com), you can do this using the `server_name` directive in NGINX.

In the NGINX configuration, you can define a separate server block for each domain. Within each block, you can specify what happens when a request is made to a specific domain. If you want to block traffic from `test.company.com`, you can create a server block for this domain and return the HTTP status code 403 (Forbidden), which indicates that the server understands the request but refuses to process it.

This configuration will allow traffic from `company.com` to pass through, while traffic from `test.company.com` will be discarded with an HTTP status code of 403.

```yaml title="NGINX configuration"
# Block traffic from test.company.com
server {
  listen 80;
...
## Performance Testing
The RUM Server is a Node.js application that can be scaled horizontally. You can run multiple instances of the RUM Server and use a load balancer to distribute the load between them. Since the RUM Server is stateless, you can run as many instances as needed.

The Docker image with Canarytrace RUM includes [k6](https://k6.io/) performance scripts. These allow you to run tests and evaluate the performance of the RUM Server based on your specific settings and the number of visits to your pages. An increased number of page visits implies more payloads being sent to the RUM Server.

```bash title="Download k6 performance scripts"
docker run --rm -it --entrypoint /bin/mv -v $(pwd):/k6 quay.io/canarytrace/rum:latest /opt/canary-rum/k6/ /k6/
```

```bash title="Print directory k6"
ls -lah k6 
-rw-r--r--@ 1 rdpanek  staff    16K 15 čvc 04:23 01-demo-data.js
-rw-r--r--@ 1 rdpanek  staff   4,3K 15 čvc 04:23 02-simply.js
drwxr-xr-x@ 5 rdpanek  staff   160B 15 čvc 04:23 data
```

- `data` contains test data for the performance scripts.
- `01-demo-data.js` is a full script that sends payloads to the [RUM Server](./rumServer) and simulates a real users.
- `02-simply.js` is a simple script that sends simple payloads to the [RUM Server](./rumServer).

```bash title="Run performance script"
export RUM_SERVER=https://<your-rum-server> && k6 run k6/01-demo-data.js --insecure-skip-tls-verify
```

These performance scripts send payloads to the `/rum` endpoint on the RUM Server. Please monitor the logs of the POD running the RUM Server and check the amount of resources used in Kubernetes.
