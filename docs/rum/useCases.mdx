...
For first time we need focus on the first load of the page and in additional we can look on the interactiv

- Use Elasticsearch indices `c.rum.metrics-*` and the label `view.duration` if you want filter manually. More information about [metrics](./rumClient#metrics).
- Kibana visualization:

Pracuje e-shop plynule?
	- `view.fps`
