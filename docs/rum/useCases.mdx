...
Website loading speed is becoming a crucial factor that influences your e-shop's placement in Google search results. E-shops with faster loading not only offer a better user experience, increasing the likelihood of conversions, but also gain more trust from search engines like Google. In such a competitive environment as online shopping, this edge can be decisive. Canarytrace RUM provides you with deep insights into the performance of your e-shop from the end-user's perspective, allows you to pinpoint loading speed weaknesses, and delivers the necessary data for optimization. Investing in Canarytrace RUM can be the key to enhancing your e-shop's visibility in search engines and boosting your revenue.
...
:::note
We will use [Kibana](./kibana) visualizations with data aggregated into graphs and indices for filtering and searching data. Please open RUM visualizations or open indices with the `c.*` prefix for searching and filtering data. In both cases, set the date range to the last 24 hours or the last 7 days.
:::
...
**How long does it take for the page to fully load?**

For first time we need focus on the first load of the page and in next step we can look on the interactivity.

When we look at the graph below, the expected behavior is that even when the number of visits increases, the performance metrics should remain at the same value. If an increase in visits is accompanied by an increase in performance metrics, this indicates performance problems with your server. Poor server performance can impact the speed of your frontend.

![FPS](./assets/kibana-speed.webp)

Performance metrics such as Duration and Dom Complete mark the time it takes for your e-shop to load in a client's browser. In this example, you can see it takes approximately 3.5 seconds, which is longer than you might desire. However, this always depends on your requirements, your business, and your competition, because these metrics will have very different values for simple web pages like blogs versus more complex pages with many products, for example.

So, how fast should your e-shop load on a user's mobile phone or desktop?

:::tip
- Our [Key findings](../glossary/keyf#key-findings) might help you determine how fast your e-shop should be.
:::

Kibana offers search, filter, and aggregation capabilities for metrics directly in your `c.rum.metrics-*` index, including metrics such as [FPS](../glossary/webperf#fps), [LongTasks](../glossary/webperf#long-task), [Duration](../glossary/webperf#duration) and [Dom Complete](../glossary/webperf#domcomplete), e.g.:

- get all FPS which are lower than 40 and higher than 0: `metrics.fcp <= 40 and metrics.fcp > 0`
- get all duration higher than 2 seconds: `metrics.duration > 2000`

**How can we gather more information about our clients and their conditions?**

Open index `c.rum.metrics-*`, select date time range and filter by `metrics.duration > 2000` and result is:
- graph with label `1,418 hits` shows how often and when is duration higher than 2 seconds
- selected fields shows unique ID of the client `session.id`, duration, device type, platform and browser.

Now, you have an overview of the clients' conditions and the duration of load our e-shop on their devices. 

:::tip
If you want to improve [Duration](../glossary/webperf#duration) or other metrics, read our recommendations in the [WebPerformance Glossary](../glossary/webperf).
:::

![Kibana duration](./assets/kibana-duration-index.webp)

If you identify a problem with the duration, you can access additional information to determine which devices and browsers were affected. Of course, you can also obtain detailed information from [other metrics](./rumClient#browser-data-collected).

![Kibana statistic](./assets/kibana-statistics.webp)

If you select `session.id` (which is the [unique ID of the client](./rumClient#session)), you can observe the conditions, parameters, and metrics for a specific user. The image below shows a user journey, sorted by `session.duration` in minutes, and includes the pages visited by the user.

![Kibana user journey](./assets/kibana-userjourney.webp)

If you select the Field statistics tab, you can view statistics for the selected field. In this example, you can see that 18.6% of users stay on our e-shop for less than 1 minute, and 13.7% stay for less than 5 minutes.

In the second screen, you can see that 59% of our e-shop visits come from mobile devices and 41% from desktop devices.

![Kibana statistic](./assets/kibana-stats.webp)

:::tip
More information about how to use Kibana can be found in the [Kibana](./kibana) section.
:::

**Is the e-shop running smoothly for users?**

Visualizations in Kibana reveal that most customers have a high [FPS](../glossary/webperf#fps), and the red circles indicate that some users have experienced slowdowns in your e-shop due to the code of your e-shop on their device's browser - we call these circles [LongTasks](../glossary/webperf#long-task).

In general, aim for the highest possible FPS and the least number of LongTasks.

![FPS](./assets/kibana-fps.webp)


For aggreation and filtering FPS in Kibana you can use `c.rum.metrics-*` index and label `view.fps`.

You can focus on the metric [INP](../glossary/webperf#inp), which represents the time between the user's last interaction and the browser's response to that interaction. This metric is crucial for the user experience as it indicates how responsive the e-shop is to user input. The lower the value, the better the user experience.

![Kibana and inp](./assets/kibana-inp.webp)

In the image above, we can observe that the statistics of the INP metric are not in green, as a good INP is lower than 200ms. Yes, the majority of users experience response times under 200ms, but there is a small group of our clients who had a bad experience with the smoothness of our e-shop, as their INP was higher than 200ms.

We can further investigate by selecting the INP metric and the device type, which reveals that the majority of users experiencing a bad INP are using mobile devices.

![INP and mobile](./assets/kibana-inp-mobile.webp)

You can compare with the desktop device type.

![INP and desktop](./assets/kibana-inp-desktop.webp)

From both images above, it is evident that the majority of users experiencing a bad INP are using mobile devices. Our clients predominantly use mobile devices compared to desktop devices. 

![INP and device type](./assets/kibana-inp-device-type.webp)

For aggreation and filtering INP in Kibana you can use `c.rum.metrics-*` index and label `metrics.inp`.

**Which pages are the most visited?**

You can create your own visualizations in Kibana, allowing you to visualize the most visited pages in our e-shop and the devices that were used. In the image below, you can observe that the most visited page is the basket, and the majority of the visits were made from desktop devices.

![Most visited pages](./assets/kibana-visited-pages-and-devices.webp)


This visualization uses `view.pathname` and `attributes.device.type` labels from the `c.rum.metrics-*` index.

And in this donut graph, we can observe that the top referrer is Google, and the most visited page from Google is a some product page. With this information, we can evaluate our marketing strategy.

![Most referer](./assets/kibana-referer.webp)

This visualization uses `view.referrer` and `attributes.browser` labels and `view.referer.keyword : * AND NOT view.referer.keyword : ""` filter from the `c.rum.metrics-*` index.

**How long does the user stay on the page?**

It's interesting to know how long users spend on the page. You can observe that clients in our e-shop typically stay from under 1 minute to 10 minutes.

![User duration](./assets/kibana-user-duration.webp)

This visualization uses `session.duration` label from the `c.rum.metrics-*` index.


**Which actions are most frequently performed by the user on the page?**

In our e-shop, these actions are associated with buttons such as `crum-action="header-menu-coptery"` and `crum-action="header-menu-frames"`, allowing us to track user interactions on our page.

In the image below, we can observe that our clients most frequently click on the category button labeled motors.

![User actions](./assets/kibana-actions-graph.webp)

This visualization uses `view.actions.name` label from the c.rum.metrics-* index.

**When do users see the important parts of the page?**

Understanding the timing of when users sees the important elements of the page on their devices is of paramount importance. For instance, users typically engage with critical components such as our products, catalog, search input, "Add to Basket" button, and AdWords banner. It is imperative that these vital elements of our e-shop are displayed rapidly to ensure a seamless user experience.

You can locate the so-called [Hero Elements](./rumClient#heroelements) and their corresponding timings within the `c.rum.metrics-*` index. In this specific example, we can observe that critical elements of our e-shop, such as the top menu, are displayed within 412ms, and the products table is visible within 733ms after the initiation of loading our e-shop page.


![Hero Elements](./assets/kibana-hero-elements-index.webp)

### Technical analysis of our e-shop

**How quickly did the resources load?**
For successful loading of the page, our e-shop needs resources such as images, CSS, JavaScript, and third-party scripts. The faster the resources load, the quicker our e-shop will load, providing a better user experience.

We can search for interesting information about requests that are important to monitor. Open the `c.rum.entries-*` index and apply filters, such as the `name` of the request, and then sort by `responseTime`. With the expanded arrow, we can view details about the requests, such as specific timings like 'time to first byte' [(TTFB)](../glossary/webperf#ttfb) or [response time](../glossary/webperf#ttfb).

![Kibana find resources](./assets/kibana-find-resources.webp)


**What resources are being loaded?**

We can obtain this information through visualization, which displays the types of requests over time along with their Encoded Body Size. In this graph, we can observe how many the various types of requests our e-shop receives, such as CSS, JavaScript, Images, etc. We can as well as identify any bad requests.

It's always better to have a smaller number of requests, and when releasing a new version, the count of requests should remain the same, never increasing.

![Resources](./assets/kibana-resources-graph.webp)

This visualization uses `view.resourceTypes.css.count`, `view.resourceTypes.fetch.count`, `view.resourceTypes.images.count` and `view.resourceTypes.other.count` from the `c.rum.metrics-*` index.



**How many resources are being used in the user's browser?**

An interesting piece of information is the amount of memory utilized by the user's browser. Higher memory consumption in users' browsers can lead to a poor user experience, such as slow page loading, browser crashes, or even unsmooth scrolling on our e-shop. In the graph below, we can observe memory consumption on both mobile and desktop devices. It's important to note that the web page for desktop devices tends to be more complex than that for mobile devices.

Increased memory consumption can be attributed to a memory leak in our e-shop, which may arise from poorly implemented JavaScript. In extreme cases, higher memory consumption can result in issues with response time, causing delays.

![Used resources](./assets/kbana-memory-used.webp)

This visualization uses `view.usedMemory` label with filters `attributes.device.type : "desktop"` and `attributes.device.type : "mobile" ` from the `c.rum.metrics-*` index.


**Is everything working properly?**

This table can reveal whether users encountered any issues while shopping. Errors can act as obstacles in the user journey and hinder successful conversions. In the image below, we can observe the types of errors and their frequency/count.

![Errors](./assets/kibana-errors.webp)

This visualization uses `view.console.type` and `view.console.value` labels from the `c.rum.metrics-*` index.


**Devices and Browsers Used by Our Clients**

For strategic planning, it's important to understand which devices and browsers our clients are using. In the image below, we can observe that the majority of our clients use Chrome version 115 on desktop devices.

![Devices and browsers](./assets/kibana-devices-browsers.webp)

This visualization uses `attributes.browser`, `attributes.browserVersion` and `attributes.device.type` labels from the `c.rum.metrics-*` index.


**How Many Users Are Currently Visiting Our E-shop?**

The count of active users is crucial information for comparison with other metrics. For instance, if we have a high number of active users but a low conversion rate, we can deduce that the issue lies not in the quantity of users, but rather in the user experience.

Another use case is to compare the number of active users with other metrics, such as duration. Through this comparison, we can determine if the user experience deteriorates when more users are on the page. If the duration increases with the number of users, it suggests that the issue might be related to our server.

![Session](./assets/kibana-sessions.webp)

This visualization uses `session.id` label from the `c.rum.metrics-*` index.

**What is the speed of the backend and frontend?**

Another interesting piece of information is the speed of both the backend and frontend. This helps us identify if our e-shop is running slowly and pinpoint whether the issue lies in the frontend or in the backend.

In this graph and over time, we can observe that our backend is consistently slower than expected. As a result, the load time cannot be lower than 2 seconds, considering the backend takes longer than 2 seconds to respond. 

Throughout the entire day, the backend maintains a speed lower than 2 seconds.

Also, consider that you must select the right conditions, meaning mobile or desktop usage, peak times, etc.

![Backend vs frontend](./assets/kibana-backend-frontend.webp)

This visualization uses `metrics.ttfb` and `metrics.fcp` label from the `c.rum.metrics-*` index.


**What Devices, Platforms, and Connection Types Do Users Use?**

Crucial information includes the types of devices, platforms, and connections utilized by our clients. In the image below, we can observe that the majority of our clients use desktop devices with the Windows operating system and an efficient 4G connection. This visualization can be expanded with additional attributes such as [downlink](./rumClient#attributes), [rtt](./rumClient#attributes), etc.

![Device](./assets/kibana-platform-os-connection.webp)

This visualization uses `attributes.devices.type`, `attributes.device.platform` and `attributes.connection.effectiveType` labels from the `c.rum.metrics-*` index.


**How Much Data Is Being Transferred?**

Crucial information regarding our e-shop involves the volume of data being transferred. In the image below, we can observe the data transfer amounts over time. We have the option to select the type of data we wish to monitor, such as HTML, CSS, JavaScript, images, etc., or specific resources like fetch requests, XHR requests, etc.

![Encoded body size](./assets/kibana-encoded-body-size.webp)

This visualization uses `encodedBodySize` label from the `c.rum.entries-*` index.

**How Many Resources Are Blocking the Rendering of Our E-shop?**

This visualization enables us to monitor the quantity of resources that obstruct the rendering process of our e-shop.

Render-blocking requests play a significant role in performance, as they hinder the complete rendering of the page. However, identifying them can occasionally be challenging. Fortunately, Chrome has begun providing more detailed reports on whether a request qualifies as render blocking.

![Blocking resources](./assets/kibana-blocking-rendering.webp)


In conclusion, Canarytrace RUM serves as a versatile and indispensable tool for enhancing the performance and user experience of your e-commerce platform. By closely monitoring loading times, user interactions, and various performance metrics, you gain a comprehensive understanding of how your online store operates in the real world. Armed with this actionable data, you can pinpoint areas for improvement, optimize user journeys, and ensure that your customers enjoy seamless and satisfying shopping experiences.

With Canarytrace RUM, you're equipped to fine-tune your website's performance, increase user engagement, and boost conversion rates. By continuously refining your e-commerce site based on real user behavior, you'll be able to stay ahead of the competition and build long-lasting relationships with your customers.

Implementing Canarytrace RUM is a straightforward process. Start by adding the provided init script to your pages or reusable components to begin collecting essential data. With this foundational step in place, you'll unlock a wealth of insights into user behavior, performance bottlenecks, and opportunities for optimization.

Remember, your users' satisfaction and the success of your e-shop go hand in hand. Utilize Canarytrace RUM's features to their fullest extent, leveraging HeroElements to measure critical content visibility, tracking user actions to identify friction points, and utilizing events to gather context-rich information. By embracing these capabilities, you're well-positioned to elevate your e-commerce platform to new heights.
