...
The `reportLabels` parameter serves as a label selector for `c.listener.events-*`. This parameter determines which labels from the selected index should be included in the events index. It is used to enrich the documents array in `c.listener.events-*` with specific information from a particular document that violates the rule. The labels stated in `reportLabels` must correspond to those contained in the relevant index to which the rule applies.
...
The Listener Agent can be configured using environment variables, and these configurations can be changed in the `k8s/deployment.yaml` and `k8s/secret.yaml` files.


### Elasticsearch

- `ELASTIC_CLUSTER = 'https://abc.eu-central-1.aws.cloud.es.io'` - Elasticsearch cluster endpoint.
- `ELASTIC_HTTP_AUTH = 'elastic:ABCQ36RGUw5p6oAj5s123456'` - Elasticsearch HTTP authentication, username:password.
- `ELASTIC_REQUEST_TIMEOUT = '10000'` - Elasticsearch request timeout in milliseconds. Default `10000`.
- `INDEX_PREFIX = 'c.'` - Elasticsearch index prefix, e.g. `c.listener-*`. Default `c.`.
- `QUERY_SIZE = '1000'` - Elasticsearch [paginate search size](https://www.elastic.co/guide/en/elasticsearch/reference/current/paginate-search-results.html). Default `1000`.
- `ELASTIC_OBSERVABILITY = 'allow'` - Print all Elasticsearch queries into stdout. Default `no`.
- `KIBANA_ENDPOINT = 'https://abc.kb.eu-central-1.aws.cloud.es.io:9243'` - Kibana endpoint for creating links to Kibana.

### Reporters

- `SLACK_WEBHOOK_URL_INTERNAL` - Slack webhook URL to which the reporter will send critical events, e.g., problems from [internal checks](#first-phase-internal-checks).
- `SLACK_WEBHOOK_URL` - Slack webhook URL to which the reporter will send events from rule analyzers and state of the errors queue.
- `EMAIL_SMTP_SERVER` - SMTP server for sending emails.
- `EMAIL_SMTP_PORT` - SMTP port for sending emails.
...
- `EMAIL_SENDER = 'your@company.com'` - Email address from which the emails will be sent. Default `notifications@canarytrace.com`.
...
### Overviews
Internal settings. Therefore, you may not need to change them.
- `EMERGENCY_AFTER_MIN = 60` - Send an emergency overview after 60 minutes. The default is `60`.
- `STAGE1_AFTER_MIN = 180` - Send an overview of errors every 180 minutes. The default is `180`.
- `STAGE1_REPETETION = 20` - Reminder about an error when the same error has been found more than 20 times. The default is `20`.

### Internal Checks
- `CANARY_HEALTHCHECK = 'allow'` - Checks whether Canarytrace Synthetic is running and sending data to Elasticsearch. Default: `allow`.
- `CANARY_HEALTHCHECK_MINUTES = '5'` - Allows you to set a threshold for the internal check. By default, the Listener will notify you if no new data has been received from Canarytrace for more than 5 minutes, which means that Canarytrace has not been running for more than 5 minutes. Default: `5` minutes. 
- `DM_HEALTHCHECK = 'allow'` - Checks if the Data Manager is installed. Default: `allow`.
- `DM_LAST_HOURS = '48'` - Ensures that the Data Manager has been operational within the last 48 hours. Default: `48`.
- `LIFECYCLE_CHECK = 'allow'` - Useful for verifying that Canarytrace Synthetic is operating correctly. Default: `allow`.
- `ELASTIC_HEALTHCHECK = 'allow'` - Checks the health status of Elasticsearch. Default: `allow`.
- `ELASTIC_NODES_HEALTHCHECK = 'allow'` - Monitors the health status of Elasticsearch nodes. Default: `allow`.


:::info

At the same time, we do not recommend changing `CANARY_HEALTHCHECK_MINUTES` threshold in any way. Canarytrace should run as often as possible, and this is our recommendation for higher quality of your web application.

:::

### Others
- `LICENSE = 'ABCDE-12345-ABCDE-12345-ABCDE-123'` - Place your license key here.
- `ENV_PRINT = 'allow'` - Print all environment variables into stdout.
- `TAGS = 'monitoring;production'` - Tags that will be added to all events and errors.

### Rules
Internal settings. Therefore, you may not need to change them.
- `PRINT_RULES='allow'` - Print evaluated rules into stdout, default `no`.
- `CUSTOM_RULES_PATH = './rules/custom.yaml'`
- `INTERNAL_RULES_PATH = './rules/internal.yaml'`
- `RUM_RULES_PATH = './rules/rum.yaml'`
