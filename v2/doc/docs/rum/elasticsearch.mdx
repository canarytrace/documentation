...
## Why Elasticsearch
The [RUM Server](./rumServer.md) store data arrived from the [RUM Client](./rumClient.md) into [Elasticsearch](https://www.elastic.co/elasticsearch/). We use the Elasticsearch as a database for analyze data from the Canarytrace Synthetic and from the Canarytrace RUM. 

Elasticsearch is a timeseries database that contains indices with data from the [RUM Server](./rumServer.md). Each index is analogous to a table in a relationship database, such as MSSQL. The naming convention is `c.index-name.*`, where `c.` is the namespace for data from the Canarytrace toolset, such as Canarytrace RUM. `index-name` contains data for the domain, for example, `c.metrics.*` contains metrics, and the asterisk represents the date because a new index is created every day with the date in its name."
...
The [RUM server](./rumServer.md) sends data to Elasticsearch into to four indices. If an index does not exist, Elasticsearch will automatically create it when the RUM server sends the first set of data.
